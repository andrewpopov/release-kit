...
This package manages its `CHANGELOG.md` with its own tooling
([`release-kit`](https://github.com/andrewpopov/release-kit)) using a
fragment-based flow instead of hand-editing the changelog directly. (Since
release-kit cannot depend on itself, the commands below invoke the local
build directly — see `release-kit.config.js` and the `release:*` scripts in
`package.json`.)
...
- **Add a fragment for your change:** `npm run release:note -- --kind fixed --slug short-slug --summary "User-facing summary"`
  ...
  write one by hand following the format above). Under the hood this runs
  `node dist/cli.js note`.
