...
function parseArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg === '--help' || arg === '-h') {
            ...
            parsed[key] = arg === '--root' ? node_path_1.default.resolve(next) : String(next).trim();
            index += 1;
        }
        else if (arg === '--force') {
            parsed.force = true;
...
function loadConfigFromFile(rootDir) {
    const candidates = ['release-kit.config.js', 'release-kit.config.cjs'];
    for (const candidate of candidates) {
        const configPath = node_path_1.default.join(rootDir, candidate);
    ...
    throw new Error(`No release-kit.config.js (or .cjs) found in ${rootDir}. Create one that exports a ReleaseKitConfig (see defineConfig).`);
}
...
function run(argv = process.argv.slice(2), stdout = process.stdout, stderr = process.stderr, options = {}) {
    ...
    switch (parsed.verb) {
        case 'note': {
            const filePath = (0, fragments_1.writeNewFragment)(config, { kind: parsed.kind, slug: parsed.slug, summary: parsed.summary });
            stdout.write(`Created ${node_path_1.default.relative(rootDir, filePath)}\n`);
            return 0;
        }
        case 'notes': {
            const version = (0, publish_1.resolveVersion)(config, parsed.version);
            config.versionStrategy.assert(version);
            stdout.write((0, render_1.renderReleaseNote)(config, {
                version,
        ...
        case 'bump': {
            const result = (0, publish_1.bumpVersion)(config, { version: parsed.version });
            stdout.write(`Bumped ${config.versionLabel.toLowerCase()} ${result.previousVersion} -> ${result.version}\n`);
            return 0;
        }
        case 'publish': {
            const result = (0, publish_1.publishRelease)(config, {
        ...
        case 'cut': {
            const result = (0, publish_1.cutRelease)(config, {
                version: parsed.version,
                date: parsed.date,
                commit: parsed.commit,
                force: parsed.force,
        ...
        case 'check': {
            const version = (0, publish_1.resolveVersion)(config, parsed.version);
            const result = (0, publish_1.validateReleaseState)(config, version);
            if (!result.ok) {
        ...
        case 'verify-bins': {
            ...
            if (process.platform === 'win32') {
                stdout.write('release:verify-bins: skipped on Windows - NTFS carries no executable bit, so every packed entry reads as 644 here. Run this on Linux or macOS before publishing.\n');
                return 0;
            }
